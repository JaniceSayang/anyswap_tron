 ...
 * {TERC20-allowance} without having to send a transaction, by signing a
...
interface IAnyswapV3ERC20 is TERC20, IERC2612 {
...
library SafeERC20 {
    ...
    function safeTransfer(TERC20 token, address to, uint value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(TERC20 token, address from, address to, uint value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    function safeApprove(TERC20 token, address spender, uint value) internal {
        require((value == 0) || (token.allowance(address(this), spender) == 0),
            "SafeERC20: approve from non-zero to non-zero allowance"
        );
        callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, value));
    }
    function callOptionalReturn(TERC20 token, bytes memory data) private {
...
contract AnyswapV6ERC20 is IAnyswapV3ERC20 {
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimals, address _underlying, address _vault) {
        ...
        // TODO 底层代币精度 波场不确定能够通过此方式获取到
        if (_underlying != address(0x0)) {
            //require(_decimals == TERC20(_underlying).decimals());
        }
    ...
    function deposit() external returns (uint) {
        uint _amount = TERC20(underlying).balanceOf(msg.sender);
        TERC20(underlying).safeTransferFrom(msg.sender, address(this), _amount);
        return _deposit(_amount, msg.sender);
    }

    function deposit(uint amount) external returns (uint) {
        TERC20(underlying).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function _withdraw(address from, uint amount, address to) internal returns (uint) {
        _burn(from, amount);
        TERC20(underlying).safeTransfer(to, amount);
        return amount;
    }
    ...
    function _mint(address account, uint256 amount) internal {
        require(account != address(0), "TERC20: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal {
        require(account != address(0), "TERC20: burn from the zero address");
